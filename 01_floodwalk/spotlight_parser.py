...
class FileMetaDataListing:
    ...
    def ParseItemV1(self, properties, id):
        ...
        while self.pos < self.size:
            ...
            if prop_index == 0:
                log.warning(
                    f"Maybe something went wrong, skip index was 0 @ {filepos} or rest of struct ({self.size - self.pos} bytes) is slack")
                break
    ...
    def ParseItem(self, properties, categories, indexes_1, indexes_2):
        ...
        try:
            ...
            while self.pos < self.size:
                ...
                if prop == None:
                    ...
                    if value_type == 0:
                        ...
                        if prop_type & 2 == 2:
                            value = [self.ReadVarSizeNum()[0] for x in range(
                                2)]  # Looks to be just 2 bytes _kMDItemRecentAppSearchEngagementRenderPositions
                        else:
                            value = self.ReadVarSizeNum()[0]
                    elif value_type == 7:
                        ...
                        if prop_type & 0x80 == 0x80:
                            ...
                            value = self.ReadStrings(dont_decode=True if prop_name != 'kMDStoreProperties' else False)[
                                0]
                        else:
                            ...
                            if prop_type & 3 == 3:  # in (0x83, 0xC3, 0x03): # ItemKind
                                ...
                                if value == None:
                                    ...
                                    for v in value:
                                        ...
                                        if cat == None:
                                            log.error(
                                                'error getting category for index={}  prop_type={}  prop_name={}'.format(
                                                    v if v is not None else "UNKNOWN", prop_type, prop_name))
                                        else:
                        ...
                        if prop_name != 'kMDStoreAccumulatedSizes':
                            log.info(
                                "Pos={}, Unknown value_type {}, PROPERTY={}, PROP_TYPE={} ..RETURNING!".format(filepos,
                                                                                                               value_type,
                                                                                                               prop_name,
                                                                                                               prop_type))
                        return
                    if prop_name in self.meta_data_dict:
                        log.warning(
                            'Spotlight property {} had more than one entry for inode {}'.format(prop_name, self.id))
                    self.meta_data_dict[prop_name] = value
            ...
            if bytes_left == 0:
                log.warning(
                    f'Had exception, exhausted data but entry may be incomplete at {self.file_pos}! Last prop={prop_name}, Last prop pos={prop_location}')
            else:
...
class SpotlightStore:
    ...
    def ParseIndexesFromFileData(self, data_content, offsets_content, header_content, dictionary, has_extra_byte=False):
        ...
        for index, offset in offsets:
            if index % 100000 == 0:
                log.debug(
                    f'{index}, {100 - ((len(offsets) + 1 - index) * 100 / (len(offsets) + 1))} % done (ParseIndexesFromFileData)')
            pos = offset
            if pos >= data_len:
                log.error(f'Index ({index}) Offset ({offset})> filesize ({data_len}) in ParseIndexesFromFileData()')
                continue
            entry_size, entry_size_bytes_moved = SpotlightStore.ReadIndexVarSizeNum(
                data_content[pos: min(pos + 9, data_len)])
            ...
            if entry_size - index_size - entry_size_bytes_moved != 0:
                log.debug(
                    f"diff in entry_size and index_size, diff={entry_size - index_size - entry_size_bytes_moved} for index={index}, offset={offset}, entry_size=0x{entry_size:X}, index_size=0x{index_size:X}")
            if has_extra_byte:
                pos += 1
